...
export default function NavbarAgricultor({ nombre: nombreProp, rol: rolProp }) {
  const { user } = useContext(AuthContext);
  const nombre = nombreProp || user?.displayName || (user?.email || '').split('@')[0] || 'Usuario';
  const rol = rolProp || (user?.role === 'AGRICULTOR' ? 'Agricultor' : user?.role === 'ADMIN' ? 'Admin' : 'Comprador');
